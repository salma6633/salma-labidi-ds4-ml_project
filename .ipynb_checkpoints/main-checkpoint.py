...
es = Elasticsearch([{'scheme': 'http', 'host': 'localhost', 'port': 9200}])  # Ajoute 'scheme' ici
if es.ping():
...
def main():
    parser = argparse.ArgumentParser(description='Pipeline ML pour la prédiction de churn avec MLflow.')
    parser.add_argument('--step', type=str, help='Étape à exécuter: prepare, train, evaluate, save, load, staging, production')
    ...
    if args.run_all:
        if not args.data:
            raise ValueError("Le chemin du fichier de données est requis pour '--run-all'.")
        ...
        if not args.data:
            raise ValueError("Le chemin du fichier de données est requis pour 'prepare'.")
        print("Préparation des données...")
        X_train, X_test, y_train, y_test = mp.prepare_data(args.data)
        with open('customer_churn_model.pkl', 'wb') as f:
            pickle.dump((X_train, X_test, y_train, y_test), f)
        ...
        with mlflow.start_run(run_name=generate_random_name()):
            ...
            mlflow.log_metrics(metrics)
            log_metrics_to_es(metrics)  # Appel à la fonction pour envoyer vers Elasticsearch
