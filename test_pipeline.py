...
class TestDataPreparation(unittest.TestCase):
    def test_file_exists(self):
        ...
        self.assertTrue(
            os.path.exists(data_file), f"Le fichier {data_file} est introuvable."
        )
    ...
    def test_prepare_data(self):
        ...
        self.assertEqual(
            len(X_train),
            len(y_train),
            "Les tailles de X_train et y_train ne correspondent pas.",
        )
        ...
        self.assertEqual(
            len(X_test),
            len(y_test),
            "Les tailles de X_test et y_test ne correspondent pas.",
        )
    ...
    def test_no_missing_values(self):
        ...
        self.assertFalse(
            ...
            "Des valeurs manquantes existent dans X_train.",
        )
        self.assertFalse(
            X_test.isnull().values.any(), "Des valeurs manquantes existent dans X_test."
        )
        self.assertFalse(
            y_train.isnull().values.any(),
            "Des valeurs manquantes existent dans y_train.",
        )
        self.assertFalse(
            y_test.isnull().values.any(), "Des valeurs manquantes existent dans y_test."
        )
    ...
    def test_column_names(self):
        ...
        self.assertTrue(
            all(
                isinstance(col, str) or pd.api.types.is_numeric_dtype(X_train[col])
                for col in X_train.columns
            ),
            "Certaines colonnes de X_train ne sont pas du bon type.",
        )

        # Vérifier que y_train est une série avec des étiquettes uniques (si c'est un problème de classification)
        self.assertTrue(
            len(y_train.unique()) > 1,
            "y_train ne contient pas assez de classes uniques.",
        )
